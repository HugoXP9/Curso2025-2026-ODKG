...
class Report:
    ...
    def validate_task_06_01(self, g):
        ...
        if self.is_subClassOf(g, professorURI, personURI) and             self.is_subClassOf(g, associateProfessorURI, professorURI) and             self.is_subClassOf(g, interimURI, associateProfessorURI) and             self.is_subClassOf(g, fProfessorURI, professorURI):
    ...
    def validate_07_01(self, result, task):
        ...
        for c,sc in result:
            ...
            if sc == None and "Person" not in str(c) and "Animal" not in str(c):
                ...
                error = True
            if "Person" not in str(c) and "Animal" not in str(c)             and "Professor" not in str(c) and "Student" not in str(c)             and "FullProfessor" not in str(c) and "AssociateProfessor" not in str(c)             and "AssociateProfessor" not in str(c) and "Instructor" not in str(c)             and "InterimAssociateProfessor" not in str(c):
